...
<xsl:stylesheet version="2.0" xmlns:bachf="http://www.sitzextase.de"
    ...
    <!-- this template takes individual words as input, most likely output from the funcStringTokenize -->
    <xsl:template name="funcStringTranscribeIjmesToArabic">
    ...
    <xsl:template name="funcStringArabicTransliterateSingleCharacters">
        ...
        <xsl:element name="tei:c">
            <xsl:attribute name="xml:lang" select="'ar'"/>
            <xsl:value-of
                select="translate($pInput,$vStringTranscribeFromIjmes,$vStringTranscribeToArabic)"/>
        </xsl:element>
    </xsl:template>
